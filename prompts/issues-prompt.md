...
4. **Implement** — make the change, treating issue #{{ISSUE_NUMBER}} as the spec. Write or update tests alongside code when a behaviour change warrants coverage. Any test file you add or modify must actually fail against the pre-change code — Phoebe automatically re-runs it against the tree before your commits and discards this attempt without opening a PR if it already passes there, since that proves nothing about the fix. If this repo ships an `implement` (or equivalent) workflow skill under `.claude/skills/`, read and follow it; otherwise apply your own tight edit → test loop.
