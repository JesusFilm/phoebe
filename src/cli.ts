    ...
    if (arg !== undefined && !arg.startsWith("-")) {
      ...
      throw new Error(
        `Unknown command \`${arg}\` for \`phoebe\`${version === null ? "" : ` (phoebe-agent v${version})`}. ` +
          `Known commands: boot, init, status, list, purge, upgrade, doctor, migrate, stop, start, pipelines, sweep-state. If \`${arg}\` was added in a newer ` +
...
export async function runCli(): Promise<void> {
  ...
  if (args[0] === "init") {
    ...
    // The one consent question (#474), asked on a TTY before the scaffold is
    ...
    try {
      const report = runInit({
        ...
        reportingMaintainers: consent,
      });
      process.stdout.write(formatInitReport(report, parsed.targetDir));
    } catch (error) {
      await reporter.report({ phase: "init", level: "error", error });
      throw error;
    }
    return;
  }
