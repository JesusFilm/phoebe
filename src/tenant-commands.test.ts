...
describe("listTenants", () => {
  ...
  test("pipeline lines come from the real in-process enumeration (#427)", async () => {
    // No `loadPipelineRows` seam: this is the enumerator the supervisor spawns
    // from, run in `list`'s own process against a config on disk.
    writeFileSync(
      join(configDir, "phoebe.config.ts"),
      `export default { workspace: { tenants: ["rows"] } };\n`,
    );
    mkdirSync(join(configDir, "rows"), { recursive: true });
    writeFileSync(join(configDir, "rows", "phoebe.config.ts"), tenantConfigSource("acme/rows"));
    writeSnapshot(join(dataBase, "acme", "rows", "state", "work"), {
      tenant: "acme/rows",
      pipeline: "work",
      currentUnits: [
        {
    ...
    writeSnapshot(join(dataBase, "acme", "rows", "state", "intake"), {
      tenant: "acme/rows",
      pipeline: "intake",
      currentUnits: [],
      waitingForSlot: true,
    ...
    mkdirSync(join(dataBase, "acme", "rows", "state", "old"), { recursive: true });
    // Not a pipeline: the tenant's clone lock lives in the same directory.
    mkdirSync(join(dataBase, "acme", "rows", "state", "clone.lock"), { recursive: true });
