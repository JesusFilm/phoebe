...
import { listPipelines, type LoadPipelineRows, type PipelineListing } from "./pipeline-listing.ts";
...
export type TenantListing = {
  ...
  /**
   * One line per pipeline beneath the tenant row (#427): the rows the
   * supervisor would spawn, plus any `state/<name>/` directory no row
...
/** Legend line for stale pipeline dirs — the row analogue of undeclared (#427). */
...
async function listingForLive(opts: {
  ...
  envPath?: string;
  disabled?: boolean;
  loadPipelineRows?: LoadPipelineRows;
  ...
  return {
    ...
    pipelines: await listPipelines({
      ...
      ...(opts.loadPipelineRows !== undefined ? { loadRows: opts.loadPipelineRows } : {}),
    }),
 ...
 * there is no row set to enumerate, and the snapshots are the only evidence of
...
export type TenantDiscoverySeams = {
  ...
  /** Enumerate a tenant's pipeline rows; defaults to the in-process enumerator (#427). */
  loadPipelineRows?: LoadPipelineRows;
};
...
function definedSeams(seams: TenantDiscoverySeams): TenantDiscoverySeams {
  return {
    ...
    ...(seams.loadPipelineRows !== undefined ? { loadPipelineRows: seams.loadPipelineRows } : {}),
  };
}
      ...
      if (tenant !== undefined) {
        ...
        listings.push(
          await listingForLive({
            ...
            disabled,
            ...(opts.loadPipelineRows !== undefined
              ? { loadPipelineRows: opts.loadPipelineRows }
              : {}),
          }),
        );
        continue;
...
export async function listTenants(
  ...
  if (enumeration !== null) {
    return listWorkspaceTenants({
      ...
      ...(opts.loadPipelineRows !== undefined ? { loadPipelineRows: opts.loadPipelineRows } : {}),
    });
  }
 ...
 * account for. Its pipeline lines do — a solo deployment runs rows like any
