...
describe("pipelineState", () => {
  ...
  test("an in-flight unit outranks a parked pass", () => {
    // Both flags set: a pass topping the pipeline up can be waiting on a second slot
...
describe("isWedged", () => {
  ...
  test("an idle pipeline is never wedged, however old", () => {
...
describe("listPipelines", () => {
  const pipelines = (facts: PipelineFacts[]) => () => facts;

  test("enumerated pipelines keep declaration order; unknown state dirs follow as stale", async () => {
    writeSnapshot("work", {
    ...
    expect(listings).toMatchObject([
      ...
      // never reads as an abandoned pipeline.
  ...
  test("a unit past its budget plus the pipeline's poll interval is wedged", async () => {
    writeSnapshot("work", {
  ...
  test("an enumeration that throws falls back to disk rather than inventing pipelines", async () => {
    writeSnapshot("work");
    const listings = await listPipelines({
      configPath: "/tenant/phoebe.config.ts",
      stateDir,
  ...
  test("no state dir at all: the enumerated pipelines still list", async () => {
    const listings = await listPipelines({
      configPath: "/tenant/phoebe.config.ts",
