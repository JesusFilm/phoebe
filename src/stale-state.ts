// The stale-state sweep (#426) — what reclaims a pipeline's disk once no row
// answers for it.
//
...
// deletes disk a row is not currently using.
//
// Four properties, all from #411:
//
//   - **Stateless.** Orphanhood is a diff of disk against the current row
...
//     `disabled` row (still enumerated) is *stopped*, not orphaned; a rename is
...
//     for exactly the duration of its unit, so locked-by-a-live-row is
...
// sweep never reaches into a live row's `state/` dir, where a tmp file between
...
import { rowOwnedKinds } from "./pipeline-row.ts";
...
/** The name and kind axes of the current row enumeration — the whole of "owned". */
export type PipelineOwnership = {
  /** Every row the enumeration produces, `disabled` ones included. */
  pipelines: ReadonlySet<string>;
  /** Every kind some enumerated row owns, `disabled` ones included. */
 ...
 * whether a row needs the clone — and assembles a registry per row to do it.
 ...
 * already spells out.
 *
 * `rowOwnedKinds` counts kinds a row has switched off, which is the point:
...
export function pipelineOwnership(config: PhoebeConfig): PipelineOwnership {
  ...
  for (const [name, row] of Object.entries(config.pipelines)) {
    pipelines.add(name);
    for (const kind of rowOwnedKinds({ pipelines: config.pipelines, pipeline: name, row })) {
      kinds.add(kind);
    }
  }
 ...
 * a tenant every row of which runs `scratch` kinds never has one (#418), and
 ...
 * touch — one a live row holds, or one locked by someone whose lock reason
...
function classifyWorktree(
  ...
  // The clone itself is the first row of `git worktree list`, and anything
  ...
  const why =
    owner !== null
      ...
      : kind !== null && !ownership.kinds.has(kind)
        ? `a read-only workspace for "${kind}", a kind no row owns`
        : "unlocked — no row holds it, so no unit is using it";
...
export function scanStaleState(deps: {
  ...
  // `state/<pipeline>/` — the snapshot and anything else the row wrote beside
  // it. The clone lock is a live tenant-wide mechanism, not a row's directory.
  for (const name of childDirectories(paths.stateDir)) {
    ...
    items.push({
      tier: "state",
      path: dir,
      detail: `no row named "${name}" — its status snapshot is unowned`,
      reclaim: null,
    });
  ...
  // `scratch/<kind>` — kind-keyed, so it follows the kind rather than the row.
  ...
  for (const kind of childDirectories(paths.scratchDir)) {
    ...
    items.push({
      ...
      detail: `no row owns the kind "${kind}"`,
      reclaim: null,
    });
  }
  ...
  for (const kind of childDirectories(join(paths.worktreesDir, READONLY_WORKTREES_SEGMENT))) {
    ...
    items.push({
      tier: "readonly",
      path: dir,
      detail: `a read-only workspace for "${kind}", a kind no row owns; git has no record of it`,
      reclaim: null,
    });
  }
...
export function formatSweepResult(result: SweepStaleStateResult): string[] {
  ...
  if (lines.length === 0) lines.push("nothing stale: every directory on disk has a row behind it.");
  return lines;
}
...
const SWEEP_STATE_HELP_TEXT = `phoebe sweep-state — reclaim disk no pipeline row owns

Usage:
  phoebe sweep-state [--config <path>] [--json]

Diffs this tenant's data directory against the rows its config declares and
...
The supervisor runs this at facility boot and after a row-set change. Run by
hand only while the tenant's rows are stopped: it is safe beside a live row (a
row's own worktrees are leased while it works), but it reclaims trees no row
currently holds.
`;
...
export async function runSweepStateCli(argv: readonly string[]): Promise<void> {
  ...
  if ((userConfig as { workspace?: unknown }).workspace !== undefined) {
    throw new Error(
      `${configPath} is a workspace root (it carries a \`workspace\` block). Rows — and the ` +
