...
Existence is the whole rule, so a `promptFiles` key may point outside the runtime
root. That is what a `configDir` tenant wants: `issue:
"../prompts/issues-prompt.md"` reaches the prompts at the repo root instead of
keeping a second copy under `<configDir>/prompts/` that silently misses every
prompt improvement merged afterwards.
