...
run`. The engine spawns `installCommand` (and prompt `!` expansions) with
inherited stdio, so that question reached a terminal with no operator watching
it and the unit stalled at install rather than failing; the run-timeout deadline
cannot interrupt a blocked `execSync`, so it stalled indefinitely. Those commands
now default the variable to `0`, which answers the confirmation without changing
what gets downloaded — the version still comes from the repo's own
`packageManager` field. An operator who sets the variable themselves keeps their
value.
