...
`phoebe list` prints one line per pipeline (#427). A tenant is several pipelines now, and one engine-state column read from one status file could only ever describe one of them. The tenant row keeps what is true of the whole tenant — path, slug, the config/env/data flags, the credential arm, its hold reason, `(disabled)` — and beneath it every pipeline gets an indented line of its own. The implicit `work` pipeline prints like any other; there is no collapsed form.
