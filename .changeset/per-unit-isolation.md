...
Per-unit isolation under concurrency (#423): the caveat on `concurrency` is gone. Two units of one pipeline can now hold worktrees, scratch directories and read-only trees at the same time without touching each other's, and the log says which unit every line came from.
