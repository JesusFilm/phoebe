...
function clientAnswering(answer: RelayVersion | Error): RelayClient {
  return {
    ...
    signIn: () => Promise.reject(new Error("the console asked for more than the version")),
    watchSession: () => () => {},
    setConfigField: () => Promise.reject(new Error("the console asked for more than the version")),
    setSecret: () => Promise.reject(new Error("the console asked for more than the version")),
