...
This is the view from inside **one pipeline**. A pipeline is one row of work a
...
describes what that row does with the kinds it owns. For the rows themselves, how
...
[`pipelines.<row>.kinds.custom`](configuration.md#custom-work-kinds-workkindscustom)
...
Each pass the row gathers work data for the kinds it owns, then `selectWorkUnits`
walks them in `order` order and takes units until the row's free slots are full —
...
Above one slot the pass tops the row up and waits on whichever comes first: a
unit settling, the poll interval, or a drain. See [`pipelines.md` → Units in
flight](pipelines.md#units-in-flight).

- **Persistent mode** (no flags) runs all the row's kinds and sleeps the row's
...
- **`--pipeline <name>`** picks the row; absent it, the row is `work`. All three
  flags combine.

A failed unit in persistent mode is logged and skipped; the row continues to the
...
- **`runTimeoutMs` is per kind.** The whole-unit wall-clock budget is settable
  ...
  pass and a forty-five-minute implementation run share a row without either
...
- **A factory kind checks its own prompts.** A definition needing prompt files
  ...
  factory runs during enumeration, so the throw fails that tenant's row list
...
hole in the agent allowlist, opt-in per key, with the subtractive row scrub the
pipelines map added on top so a sibling row's secret never reaches yours
...
thread is a unit with a `revision` and no `github` target, on a row with its own
`pollIntervalMs`, holding a Slack token no other row can see. The worked example
...
nudge that would let a row start work the moment its source has some, instead of
