...
**Who this is for:** maintainers of this repository, and anyone curious how an
agent-worked public repo handles untrusted contributors.
...
> you can adopt the same pattern, which is two files of wiring, but you don't have
...
reviewer, or an agent, whether the author is a maintainer, a known contributor,
or a drive-by.
...
touches the list, and on demand (below). It resolves the author, never the
...
| `vouch:unvouched` | `unknown`                        | Not in the list. Unknown rather than judged. |
...
To re-run the check on a single thread, either after a vouch lands or if a run
failed, comment:
...
- The same reasoning sets the default for issue-author credit (#198): Phoebe
  ...
  vouched ones. The label that queued it, whether `ready-for-agent` or the
  research label, already needed triage permission, so a maintainer vouched for the
...
**What is isolated (by mechanism).** The bootstrapper hands each engine child a
...
log, not in a receipt, and not to anyone who reads its volume later. That is a
promise about a relay operator, a backup and a passive compromise; it is **not**
a defence against a hostile relay, which serves the browser the code that does
the sealing and could serve code that keeps the value instead. It was never a
claim about the disk either, and holding ciphertext here would only put the key
that opens it on the same volume. What the store adds to this document is one file path, and the line
...
the App's entire installation set, which is strictly wider than any tenant's
`GH_TOKEN` or the engine-clone credential. Anyone who holds the key can mint fresh installation
