...
Any **other** `.env` edit (provider keys, identity vars) still relaunches the
child — those values are frozen into the scrubbed child env at spawn and a
relaunch is the only way to deliver them.
