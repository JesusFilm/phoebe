...
A **pipeline** is one named row of work a tenant runs, in its own process, with
...
**A kind belongs to exactly one pipeline.** Two rows naming the same kind is fatal
...
**Rows share the tenant and partition it.** One origin clone, one `state/`
...
**A row's failure is its own.** The container comes down only when every
supervised row is crash-looping at once. [Supervision and
faults](#supervision-and-faults).

A pipeline is not a work kind, not a scheduling policy inside one loop, and not a
type. There is no "intake pipeline" the framework ships. An intake row is a row
...
characters, so `#` can never appear in one. That is what lets `<tenant>#<row>` be
an unambiguous key.
...
drains that row and respawns it, and nothing else moves. Editing
...
acts on a change without relaunching anything, and an edit lands on rows already
queued for a slot.

That split is not a convenience. It is what the row fingerprint means. The
supervisor asks the engine checkout for each row's cold config, hashed, with
...
Two knobs are worth their own note. A row's declared `pollIntervalMs` **outranks**
`PHOEBE_POLL_INTERVAL_MS`, which is the fallback for a row that declares nothing.
...
owner id, and the credential lease's, so a row reclaims its own slots and its own
token when it dies.
...
JSON object per row: name, the hot knobs, `concurrency`, whether the row's kinds
...
subcommand gives every tenant one implicit `work` row, spawned without a
...
per poll, and tries again, rather than spawning a `work` row against a config
already known to be bad.

Enumeration spawns a Node process, so it runs only when the tenant config's stat
fingerprint moves. Steady state stays stat-only. The row diff then decides
everything. A row that appeared spawns, one that vanished drains, one whose own
fingerprint moved relaunches alone. A tenant fingerprint that moved with no row to
show for it is by elimination tenant-wide, a `gitIdentity` or a `repoSlug` or an
edited `.env`, and every row of that tenant relaunches, because every one of them
runs with it.

**The universality rule.** A row's death alone is never fatal. The container exits
only when every supervised row is crash-looping at once, and a fast crash counts
toward the crash-loop guard only when every row that ran that commit has
fast-crashed on it. One broken tenant cannot quarantine a commit the rest of the
fleet is running happily. A row is marked crash-looping when it dies of its own
accord inside the healthy-run window, and the mark survives a respawn, so two rows
crash-looping out of phase still add up to a verdict. Solo has one row, so the
rule reduces to what it always did.

The mechanics, including what an engine upgrade does to the row set, are
[`architecture.md` → Supervising rows](architecture.md#supervising-rows).

## Units in flight

`concurrency` above 1 means one row may hold several units at once. The loop that
does it is **rolling top-up**, not batch, and the distinction is that it never
awaits a specific unit.

Each pass tops the row up to its free-slot count, then waits on whichever of three
...
its read-only tree, and the prefix on its children's output. Two units of one row
...
The cap is **derived**, not fixed: `max(concurrency)` across the live rows. A row's
declared concurrency is reachable when nothing else is working, and the cap still
binds across rows. Every row at the default 1 derives 1, which is the fleet Phoebe
...
not. A row declaring more than the cap is not rewritten to fit. It queues, and boot
says so on one line. The number is recomputed on a reconcile that reshapes rows and
never on a hot edit, since a slot already granted cannot be recalled.

Starvation has two shapes, so there are two mechanisms.

A row holding no slot while it has work waiting is **starved**, and one
forty-five-minute unit elsewhere can keep it that way for forty-five minutes. Such
a row is granted one slot over the cap. `PHOEBE_SLOT_FLOOR_BUDGET` (default 1)
...
The other shape is a row that is served but always last. Tenants take turns, and
`priority` orders one tenant's rows against each other, higher first, ties in the
order they asked. Turns are taken per **row**, not per tenant, so declaring three
pipelines is declaring three independent streams and they queue as three. Starving
a low-priority row for as long as something higher is contending is what the knob is
for.

An intake row takes slots in the same currency as everything else. There is no
...
Concurrency](configuration.md#concurrency-the-rows-knob-and-the-fleets-cap).

## What a pipeline owns on disk

Two rows are two processes over one tenant's clone and one `state/` directory.
Ownership is partitioned rather than locked. Each thing on disk has exactly one
writer, and nothing needs electing.

`state/<pipeline>/status.json` is the row's alone. Stdout lines are tagged
`[phoebe:<owner>/<repo>:<pipeline>]`, `work` included, so match it as a prefix and
not as a fixed string. The four tracker sweeps are scoped to the kinds the row
schedules, so two rows cover every object exactly once and a row scheduling none of
a sweep's kinds skips it. The origin clone is shared, cloned once with the first
clone serialized by a lock under `state/`, and a row whose kinds all declare
...
whether that is another row or a sibling unit of the same row, and the unit that
wanted it comes back next cycle.

A lease outlives the process that took it, so a row breaks its own leases at boot
and never anyone else's, reading the pipeline segment of the reason and ignoring
the rest. If you find a locked worktree and no engine, the reason names the row and
...
The full ownership table is [`configuration.md` → What a row owns on
disk](configuration.md#what-a-row-owns-on-disk).

## Credentials per pipeline

An intake row reads a Slack token. The work row has no business with it. Neither
...
The row's key set is the union of what its scheduled kinds declared, and the
enumerator reports it alongside everything else about the row. The scrub the
supervisor then performs is **subtractive**: a row loses the keys a _sibling_ row
declared and it did not. So the intake row's Slack token never reaches the work
row's child, and a key nobody declares reaches every row exactly as before. That
...
be present and non-blank when the row's engine child starts, or that row fails boot
naming the kind and the key, and only that row, since its siblings boot. A kind
switched on later against a key nobody added stays off with a logged error rather
than taking the row down. `phoebe doctor` reports the same shortfall as a tenant
finding. And the `.env` reconcile digest is computed per row over the keys that row
...
The GitHub credential itself is not narrowed per row. The lease is per row, the
...
on a timer: at facility boot before any row spawns, and after a row-set change once
the rows it took down have drained. Both are moments when the disk in question is
provably nobody's.

It is a stateless diff of disk against the row enumeration, with no cursor and no
memory of the last sweep, which is what makes the awkward cases fall out rather
than need handling. A `disabled` row is still enumerated, so its state is
_stopped_, not orphaned. A rename is a delete plus a create. A kind that moved to
another row keeps its scratch, because ownership moved rather than ended. And an
...
branch-keyed. Locked by a live row is untouchable; orphan-locked or unlocked is a
...
never run. Mechanics: [`architecture.md` → Reclaiming what a row leaves
behind](architecture.md#reclaiming-what-a-row-leaves-behind).

## Watching pipelines

`phoebe list` keeps one row per tenant and prints one indented line per pipeline
beneath it. The implicit `work` row prints like any other, one grammar with no
...
The row set is the same enumeration the supervisor spawns from, so `list` and the
supervisor cannot disagree about what a tenant runs. A `state/<name>/` directory no
enumerated row produces prints as `(stale)`, the pipeline analogue of an
...
Each line's state comes from that row's own snapshot and nothing else. Two
snapshots are never compared. A row polling every fifteen minutes is not sick
because the row beside it wrote a second ago, and an idle row is not sick for being
idle a week.
...
`(disabled)`, but the supervisor records it without yet acting on it, so the row
...
object, a Slack thread or a row in someone's board, had nowhere to receive the
...
ordinary `work` row then picks it up as it would any other issue.
...
Both are one row's worth of custom kinds, and both fit like this.
...
**`readyLabel` added last.** The handoff to the `work` row is that label and nothing
else. The tracker is the entire interface between the two rows: no shared memory, no
queue, no message. An issue created without it is a draft the intake row can still
...
The **wake seam** is fixed and not built. An intake row should start work when a
...
- [`work-kinds.md`](work-kinds.md), the kind contract every row runs: `ctx`,
...
- [`architecture.md`](architecture.md#asking-the-engine-which-rows-a-tenant-has),
  the supervisor's mechanics. Enumeration, the row diff, the crash-loop rule, and
