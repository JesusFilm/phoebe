...
from a `file://` page is unverified. Cookies _on_ the `file` scheme itself are off:
...
2. The cookie is only _set_ in the app's jar if the app's own HTTP stack loads the
