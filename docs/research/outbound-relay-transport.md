...
- **None of the three reconnects for you in Node.** The WebSocket API has no reconnect;
  ...
  _channel_ but not the _stream_, and a broken bidi stream is the application's to reopen.
...
**gRPC.** Two separate mechanisms, and it matters which one you are relying on. The _channel_
...
But an _RPC_ is retried only under a service-config retry policy, and for a stream "Once the
...
relay's pings, so the relay also has to send _something_ the application layer can see. A
