...
- **Connection direction: the environment dials out, twice.** Outbound HTTPS to the relay to link
  ...
  The relay itself *does* make one inbound call into the environment, through the tunnel: the
...
- *The environment dials out and never listens publicly.* T3's server binds loopback and
...
- *One connection per environment, one retry owner.* The supervisor design (single owner of
...
- *Identity independent of route, link independent of process.* T3 keeps a stable environment
...
- *Cloud identity is not deployment authority.* Even with a single Google allowlist and no roles,
...
- *Signed, nonce-bound requests into the deployment.* The mint exchange (relay-signed proof,
...
- *Credential renewal without dropping the connection.* T3 learned this the hard way (two fixes
  in v0.0.39). Design the relay session so token refresh is an HTTP-side concern and the
  outbound socket outlives it.
- *Idempotent commands with receipts.* T3 does not replay mutations on reconnect; it makes each
...
- *Restricting sign-up by allowlist with an empty allowlist meaning "nobody".* Same semantics as
  #497's self-bootstrapping allowlist.

**Does not transfer.**

- *The relay as pure control plane.* T3 can keep the relay out of the data path because
  ...
  hostname, so the relay *is* the data path: it must terminate the deployment's outbound
...
- *Clerk, DPoP, the OAuth token-exchange vocabulary, and a Postgres-backed Worker.* All sized for
...
- *Client-side reconnect semantics.* T3's supervisor lives in the client because the client
...
- *Push notifications, Live Activities, managed-tunnel quotas, SSH launch, Tailscale.* Out of
  scope for #497 and each is either a hosted-product concern or a different route entirely.
- *Session TTLs.* 30-day environment sessions and 5-minute socket tickets are tuned for a phone
