...
| `priorityLabelPrefix` | `"priority:"`          | A `<prefix>bug`/`tracer`/`polish`/`refactor` label on an issue sets its priority bucket directly; `classifyPriority` falls back to title/body keyword-sniffing when no such label is present. The four labels are provisioned automatically at boot, the same way the quarantine/retry/tier labels are (`src/ensure-labels.ts`). See [`work-kinds.md`](work-kinds.md). |
| `bailTriageLabel`     | `"needs-triage"`       | Landing-zone label for a structural bail (`missing-prerequisite` / `external-dep` / `unclear-scope`). Provisioned automatically at boot like the other engine-owned labels (`src/ensure-labels.ts`), so a bail choreography that moves an issue here can never fail with "label doesn't exist".                                                                        |
...
Issues, PRs, and status identity all live on the one `repoSlug` — discovery,
blocker resolution, label transitions, claim/release comments, and the
`research` kind's ticket queue all address it, same as clone, worktree,
branch, and PR.
...
| `verifyMode`   | `"agent"`         | Who verification results come from (#166). `"agent"` (default, today's behavior): only the agent's self-report above. `"engine"`: the engine itself runs `checkCommand` then `testCommand` in the worktree after the agent exits, and that run is authoritative — the agent's report is never read. `"both"` (recommended rollout — shadow mode): same engine run, still authoritative, but a divergence from the agent's self-report is logged as a `⚠️ verification disagreement` line rather than silently discarded. `"engine"`/`"both"` add the check/test run's wall-clock to every unit — see [`performance.md`](performance.md#verifymode-set-to-engine-or-both-adds-wall-clock-to-every-unit). |
...
| `PHOEBE_BAIL_TRIAGE_LABEL`       | `bailTriageLabel`       |                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `PHOEBE_VERIFY_MODE`             | `verifyMode`            | Validated: must be `agent`, `engine`, or `both`.                                                                                                                                                                                                                                                                                                                                                         |
...
| `PHOEBE_MAX_PUSHES_PER_HOUR`     | `maxPushesPerHour`      | Validated: positive integer. Cost backstop (#168): max branch pushes this repo may make per wall-clock hour. Once spent, `conflicts`/`checks`/`reviews` are excluded from selection for the rest of the hour — see [`work-kinds.md`](work-kinds.md#push-rate-limit).                                                                                                                                     |
| `PHOEBE_RUN_COST_CAP_USD`        | `runCostCapUsd`         | Validated: non-negative number. Hard per-run dollar cap (#165) read off the provider's terminal `total_cost_usd`; `0` disables it. A run over cap is recorded as a retryable failure (same as a nonzero agent exit) instead of a success — the spend already happened, but repeat offenders ride the existing retry/quarantine machinery. Only Claude/Cursor report a dollar cost; Codex never trips it. |
| `PHOEBE_DAILY_COST_CAP_USD`      | `dailyCostCapUsd`       | Validated: non-negative number. Hard daily dollar cap (#165) summed from `resources.costUsd` on the events-v1 journal for the current UTC day; `0` disables it. Once reached, the engine idles — no kind is selected — until the UTC day rolls over.                                                                                                                                                     |
