...
// shrug off, because the caller's next act is to spawn pipelines regardless.
//
  ...
  test("counts what was reclaimed and carries the protected tier out whole", () => {
    const engine = engineAnswering({
      stdout: `noise from a kind module\n${JSON.stringify({
        version: 1,
        removed: [
          { tier: "state", path: "/data/state/intake", detail: "no pipeline", reclaim: null },
        ],
        kept: [
          {
            tier: "worktree",
...
`,
  ...
  test("a pipeline the config no longer declares loses its state directory", () => {
